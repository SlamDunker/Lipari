...
int crea_connetti(in_port_t server_port,in_addr_t server_addr){
	...
	if(sk < 0){
		cout<<"CLIENT5: Errore socket" <<endl;
		return -4;	
	}
	...
	if(r == -1){
		close(sk);
		cout<<"CLIENT5: Errore connect" <<endl;
		return -3;	
	}
	return sk;
}
...
static void handler(int signo){
	if(signo == 2)
		cout<<"CLIENT5: Catturato segnale SIGINT" <<endl;
	if(signo == 15)
		cout<<"CLIENT5: Catturato segnale SIGTERM" <<endl;	
	exit(0);
}
...
int main(int quanti, char* quali[]){
	...
	while(1){
		...
		if(disco){
			...
			strcpy(percorso,"./Client/Client5/img/");
			strcat(percorso,nome_img[casuale].data());
			cout<<"CLIENT5: scelto file " <<nome <<" da disco" <<endl;
		}
		else{
			name_service = "Get list";
			r = reg.request_service(name_service,dsc,server_port,server_addr);
			if(r != 0){
				cout<<"CLIENT5: Errore comunicazione con server_register" <<endl;
				if(r == -5)
					continue;
				exit(0);
			}
			...
			if(sk < 0 ){
				cout<<"CLIENT5: Impossibile connettersi con il server" <<endl;
				continue;
			}
			cout<<"CLIENT5: richiesta comando Get list" <<endl;
			...
			if(!ris){
				cout<<"CLIENT5: Errore invio parametri" <<endl;
				close(sk);
				continue;
			}
			...
			if((r == -1)||(a != ok)){
				cout<<"CLIENT5: Errore lettura esito" <<endl;
				close(sk);
				continue;
			}
			...
			if(!ris){
				cout<<"CLIENT5: Errore ricezione lista" <<endl;
				close(sk);
				continue;
			}
			close(sk);
			...
			if(!ris){
				cout<<"CLIENT5: Errore recupero lista" <<endl;
				continue;
			}
			quante = 0;
			cout<<"CLIENT5: Lista: ";
			...
			cout<<"CLIENT5: scelto il file " <<nome <<endl;
			...
			cout<<"CLIENT5: richiesta comando Get image" <<endl;
			...
			ris = servizio->resp.receive_response(sk,output);
			if(!ris){
				cout<<"CLIENT5: Errore ricezione immagine" <<endl;
				close(sk);
				continue;
			}
			close(sk);
			...
			ris = p->get_byte(img);
			if(!ris){
				cout<<"CLIENT5: Errore recupero immagine" <<endl;
				continue;
			}
			...
			f=fopen(percorso,"w");
			if(f == NULL){
				cout<<"CLIENT5: errore nell'aperture del file" <<endl;
				continue;
			}
		...
		if(casuale2 == 0){
			...
			cout<<"CLIENT5: richiesta comando Rotate image" <<endl;
			...
			f = fopen(percorso,"r");
			if(f == NULL){
				cout<<"CLIENT5: errore nell'aperture del file";
				continue;
			}
			fseek(f,0,SEEK_END);				
			num_char = ftell(f);			
			...
			ris = servizio->resp.receive_response(sk,output);
			if(!ris){
				cout<<"CLIENT5: Errore ricezione immagini" <<endl;
				close(sk);
				continue;
			}
			...
			cout<<"CLIENT5: richiesta comando Flip image" <<endl;
		...
		cout<<"CLIENT5: richiesta comando Store image" <<endl;
		...
		r = read(sk,&a,sizeof(a));
		if(r == -1){
			cout<<"CLIENT5:Errore lettura risultato" <<endl;
			continue;
		}
		cout<<"CLIENT5: Immagine " <<nome <<" salvata sul server"<<endl;
	}
}
